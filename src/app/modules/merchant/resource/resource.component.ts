...
import { Product, ProductAssets, ProductCategory, ProductInventory, ProductPagination } from 'app/core/product/inventory.types';
import { ResourceService } from 'app/core/resource/resource.service';
import { Resource } from 'app/core/resource/resource.types';
...
export class ResourceComponent implements OnInit {
	...
	selectedResourceForm: FormGroup;
	...
	initCreateProduct(productType: string) {
		...
		const dialogRef = this._dialog.open(AddResourceComponent, {
			panelClass: 'custom-add-resource-dialog',
			width: '1030px',
			maxWidth: '100vw',
			// maxWidth:'70vw',
			// height: '70vh',
	...
	editProduct(productId: string, resourceId: string) {

		const dialogRef = this._dialog.open(EditResourceComponent, {
			panelClass: 'custom-edit-resource-dialog',
			width: "100%",
			maxWidth: '100%',
			//height: '90vh',
			// maxHeight: '70vh',
	...
	openDayOff(resourceId: string) {
		const dialogRef = this._dialog.open(DayOffComponent, {
			panelClass: 'custom-edit-resource-dialog',
			width: "100%",
			maxWidth: '100%',
			//height: '90vh',
	...
	getProductCategoryName(productCategoryId) {
		...
		let category = this.productCategories$.find(p => p.id === productCategoryId);
