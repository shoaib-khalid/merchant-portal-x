...
export class CreateProductDiscountDialogComponent implements OnInit {
    ...
    checkDateTime() {
        ...
        const discountBody = {
            ...
            discountType: this.productDiscountStepperForm.get('step1.discountType').value
    ...
    async checkExistingDate(discountBody){
        ...
        if (status === 417 ){
            this.productDiscountStepperForm.get('step1.startDate').setErrors({dateOverlapped: true});
            this.productDiscountStepperForm.get('step1.endDate').setErrors({dateOverlapped: true});
